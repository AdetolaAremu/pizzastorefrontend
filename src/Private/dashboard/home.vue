<template>
  <div class="px-2 bg-white rounded-lg">
    <div class="font-semibold mt-10 text-xl text-center">
      Last Five Orders table
    </div>
    <table class="table-fixed w-full mt-2">
      ...
      <tbody class="text-center">
        <tr v-for="(order, index) in $store.fiveorders" :key="index.id">
          <td>{{ index + 1 }}</td>
    ...
    <div
      v-if="fiveorders.length === 0 && loading === false"
      class="text-center font-semibold my-3"
    >
...
export default {
  setup() {
    ...
    onMounted(() => store.dispatch("getLastFiveOrders"));

    return { fiveorders, toast, loading };
  },
};
</script>
